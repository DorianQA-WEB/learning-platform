...
async def tess_update_user_validation_error(
    client,
...
async def test_update_user_check_one_is_updated(
    client, create_user_in_database, get_user_from_db
):
    """
...
async def test_update_user_id_validation_error(
    ...
    response = client.patch("/user/?user_id=123", data=json.dumps(user_data_updated))
